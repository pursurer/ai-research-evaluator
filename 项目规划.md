...
### [已完成] 静态数据加载模块 (2024-12-31)
* **Implemented Functionality**: 完成任务 1.2，支持从 9 个顶会 CSV 加载论文数据
* **Key Decisions**: 
  - 采用 Pydantic v2 定义 `Paper` 和 `ConferenceData` 数据模型
  - `PapersLoader` 类支持单文件/多文件/按会议名加载
  - `clean_keywords()` 处理 JSON 字符串格式的关键词
  - 自定义异常类 `DataLoadError` 用于错误处理
* **Files Created**: 
  - `src/data/schema.py` — Paper, ConferenceData 模型
  - `src/data/loader.py` — PapersLoader 加载器
  - `src/data/cleaner.py` — 数据清洗函数
  - `src/utils/exceptions.py` — 自定义异常
  - `tests/test_loader.py` — 19 个测试用例（全部通过）
* **Tests**: 19 passed
...
### 3.1 Goal (Local)
**Purpose**: 实现 P-F-C 评估核心引擎（任务 1.3），集成 LLM 完成研究方向评分

### 3.2 Plan (Micro-steps)
1. 定义 `EvaluationResult`、`DimensionScore`、`DecisionLevel` 数据模型
2. 实现 `src/llm/base.py` — LLM 抽象基类
3. 实现 `src/llm/openai_client.py` — OpenAI 客户端
4. 实现 `src/llm/factory.py` — 客户端工厂
5. 编写评估 Prompt 模板 (`src/evaluation/prompts/`)
6. 实现 `src/evaluation/scorer.py` — ROI 评分计算器
7. 实现 `src/evaluation/engine.py` — 评估主引擎
8. 编写测试用例并验证
9. 提交代码并推送

### 3.3 Context & Files
- **Source files to create/modify**: 
  - `src/evaluation/schema.py` — EvaluationResult 等模型
  - `src/evaluation/scorer.py` — ROI 评分计算
  - `src/evaluation/engine.py` — 评估引擎
  - `src/evaluation/prompts/*.py` — Prompt 模板
  - `src/llm/base.py` — LLM 抽象接口
  - `src/llm/openai_client.py` — OpenAI 实现
  - `src/llm/factory.py` — 工厂模式
  - `tests/test_evaluation.py` — 评估测试
- **P-F-C 模型** (来自 judge.md):
  - P (35%): P1 趋势红利 + P2 叙事深度 + P3 SOTA 渗透率
  - F (40%): min(F1 算力, F2 数据, F3 周期) ← 短板计分
  - C (25%): C1 巨头避让度 + C2 研究空白区
  - ROI = 0.35×P_avg + 0.40×F_min + 0.25×C_avg
  - 熔断: F_min < 3 → 直接判定"不可行"
...
- [ ] ROI 评分计算正确（含熔断机制）
- [ ] LLM 抽象层可切换不同提供商
- [ ] Prompt 模板符合 judge.md 评估标准
- [ ] 决策建议输出正确（4 级分类）

### 3.5 Boundaries
- **Out-of-scope**: 本次不实现报告生成、趋势图表
- **Extensions**: 预留 Anthropic、本地模型支持接口
